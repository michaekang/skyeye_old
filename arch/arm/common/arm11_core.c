...
static void
arm11_set_pc (conf_object_t* opaque, generic_address_t pc)
{
...
static void
arm11_step_once (conf_object_t* opaque)
{
	//ARMul_DoInstr(state);
	arm11_core_t* core = (arm11_core_t*)opaque->obj;
	ARMul_State *state = core->state;
...
static exception_t arm11_set_regval_by_id(conf_object_t* opaque, int id, uint32 value){
	arm11_core_t* core = (arm11_core_t*)opaque->obj;
	ARMul_State *state = core->state;
	if(id == R13_IRQ)
...
fault_t
arm11_read_byte (ARMul_State *state, ARMword virt_addr, ARMword *data){
	//mem_read(short size, int offset, uint32_t * value){
	return NO_FAULT;
}
fault_t
arm11_read_halfword (ARMul_State *state, ARMword virt_addr,
			   ARMword *data){
	return NO_FAULT;
}
fault_t
arm11_read_word (ARMul_State *state, ARMword virt_addr, ARMword *data){
	return NO_FAULT;
}
static fault_t
arm11_write_byte (ARMul_State *state, ARMword virt_addr, ARMword data)
{
	return NO_FAULT;
}

static fault_t
arm11_write_halfword (ARMul_State *state, ARMword virt_addr,
			    ARMword data)
{
	return NO_FAULT;
}
...
static fault_t
arm11_write_word (ARMul_State *state, ARMword virt_addr, ARMword data)
{
	return NO_FAULT;
}
...
static conf_object_t* new_arm11_core(char* obj_name){
	...
	run->set_pc = arm11_set_pc;
	run->step_once = arm11_step_once;
	run->get_regval_by_id = arm11_get_regval_by_id;
	run->set_regval_by_id = arm11_set_regval_by_id;	
