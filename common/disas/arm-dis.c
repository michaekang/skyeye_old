...
//      status = info->read_memory_func (pc, (bfd_byte *)b, 4, info);
	*(unsigned int *)b = *(unsigned int *)info->buffer;
	status = 0;
      if (little)
...
//      status = info->read_memory_func (pc, (bfd_byte *)b, 2, info);
	status = 0;
	*(unsigned short *)b = *(unsigned short*)info->buffer;
      if (little)
	given = (b[0]) | (b[1] << 8);
      else
	given = (b[1]) | (b[0] << 8);
...
#if 1
		*(unsigned short *)b = *((unsigned short*)info->buffer + 1);
//		printf("\nb : %x\n", *(unsigned int*)b);
		status = 0;
#if 0
	      if (little)
